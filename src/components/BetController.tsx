...
type Props={amount:number;setAmount:(n:number)=>void;status:BetStatus;onAction:()=>void;onAutoAction?:()=>void;canCashOut?:boolean;disabled?:boolean;children?:ReactNode;potential?:number;autoTarget?:number;setAutoTarget?:(value:number)=>void;autoTargetLabel?:string;lastResult?:Result|null;idleLabel?:string;lockedLabel?:string;hideChildrenInAuto?:boolean}

export default function BetController({amount,setAmount,status,onAction,onAutoAction,canCashOut=false,disabled=false,children,potential,autoTarget,setAutoTarget,autoTargetLabel='Cashout At',lastResult,idleLabel='Place bet',lockedLabel='Bet active',hideChildrenInAuto=false}:Props){
 ...
 const changeMode=(next:'manual'|'auto')=>{if(autoRunning||status!=='idle')return;setMode(next)}
 const toggleAuto=()=>{if(autoRunning){setAutoRunning(false);return}if(status!=='idle'||disabled||amount>balance)return;startingAmount.current=amount;processedResult.current=lastResult;setCompleted(0);setNetGain(0);setAutoRunning(true)}
 ...
 return <aside className="glass w-full shrink-0 rounded-2xl border border-white/5 p-4 lg:w-[320px] lg:rounded-r-none">
  ...
  <div className="flex overflow-hidden rounded border border-[#365267] bg-[#0b1d2a] focus-within:border-sky-400/60"><div className="flex flex-1 items-center gap-2 px-3"><input aria-label="Bet amount" disabled={status!=='idle'||autoRunning} type="number" min="1" step="1" value={amount} onChange={event=>setAmount(Math.max(1,Math.floor(Number(event.target.value)||1)))} className="w-full bg-transparent py-3 text-sm font-bold outline-none"/><span className="text-sm font-bold text-lime">⬡</span></div><button disabled={status!=='idle'||autoRunning} onClick={()=>setAmount(Math.max(1,Math.floor(amount/2)))} className="border-l border-[#365267] px-3 text-xs font-bold text-slate-300 hover:bg-white/5">½</button><button disabled={status!=='idle'||autoRunning} onClick={()=>setAmount(Math.min(balance,amount*2))} className="border-l border-[#365267] px-3 text-xs font-bold text-slate-300 hover:bg-white/5">2×</button></div>
  {mode==='manual'?<div className="my-5 space-y-4">{children}</div>:autoTab==='leaderboard'?<Leaderboard/>:<div className={`my-4 space-y-4 ${autoRunning?'pointer-events-none opacity-80':''}`}>{hideChildrenInAuto?null:<div className="space-y-4">{children}</div>}<div className={`grid gap-2 ${setAutoTarget?'grid-cols-2':'grid-cols-1'}`}>{setAutoTarget&&autoTarget!==undefined?<AutoField label={autoTargetLabel}><div className="flex items-center rounded border border-[#365267] bg-[#0b1d2a] px-2"><input type="number" min="1.01" step=".01" value={autoTarget} onChange={event=>setAutoTarget(Math.max(1.01,Number(event.target.value)||1.01))} className="w-full bg-transparent py-2.5 font-bold outline-none"/><b className="text-slate-500">×</b></div></AutoField>:null}<AutoField label="Number of Games"><div className="flex items-center rounded border border-[#365267] bg-[#0b1d2a] px-2"><input type="number" min="0" step="1" value={games} onChange={event=>setGames(Math.max(0,Math.floor(Number(event.target.value)||0)))} className="w-full bg-transparent py-2.5 font-bold outline-none"/>{games===0?<InfinityIcon size={18} className="text-slate-400"/>:null}</div></AutoField></div><StrategyRow label="On Win" mode={onWin} setMode={setOnWin} percent={winIncrease} setPercent={setWinIncrease}/><StrategyRow label="On Loss" mode={onLoss} setMode={setOnLoss} percent={lossIncrease} setPercent={setLossIncrease}/><CreditField label="Stop on Net Gain" value={stopGain} setValue={setStopGain}/><CreditField label="Stop on Loss" value={stopLoss} setValue={setStopLoss}/><div className="rounded bg-[#294052] p-3"><div className="text-xs font-semibold text-slate-400">Net Gain on Win</div><div className="mt-1 flex justify-between font-bold"><span>{netGainOnWin.toLocaleString()}</span><span className="text-lime">⬡</span></div></div></div>}
  ...
  {mode==='manual'?<button onClick={onAction} disabled={disabled||(status!=='idle'&&!canCashOut)} className={`w-full rounded py-3.5 text-sm font-extrabold transition active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-50 ${canCashOut?'bg-amber-400 text-slate-950 hover:bg-amber-300':'bg-lime text-slate-950 hover:bg-[#c6fa66]'}`}>{label}</button>:<button onClick={toggleAuto} disabled={!autoRunning&&(disabled||status!=='idle')} className={`w-full rounded py-3.5 text-sm font-extrabold transition active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-50 ${autoRunning?'bg-red-400 text-slate-950':'bg-[#16ea37] text-slate-950'}`}>{autoRunning?`Stop Auto Bet · ${remaining}`:'Start Auto Bet'}</button>}
  <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-4 text-[10px] font-semibold uppercase tracking-wider text-slate-500"><span>{mode==='auto'?`Net ${netGain>=0?'+':''}${netGain.toLocaleString()}`:'Demo play'}</span><span>Provably fair</span></div>
 </aside>
}
