...
export default function BetController({amount,setAmount,status,onAction,canCashOut=false,disabled=false,children,potential}:Props){
 ...
 return <aside className="glass w-full shrink-0 rounded-2xl border border-white/5 p-4 lg:w-[300px] lg:rounded-r-none">
  ...
  <label className="mb-2 flex justify-between text-xs font-semibold text-slate-400"><span>Bet amount</span><span>${amount.toFixed(2)}</span></label>
  <div className="flex overflow-hidden rounded-lg border border-white/5 bg-[#0b1421] focus-within:border-sky-400/50"><div className="flex flex-1 items-center gap-2 px-3"><span className="text-sm font-bold text-lime">$</span><input aria-label="Bet amount" disabled={status!=='idle'} type="number" min="0.01" step="0.01" value={amount} onChange={e=>setAmount(Math.max(.01,Number(e.target.value)))} className="w-full bg-transparent py-3 text-sm font-semibold outline-none"/></div><button disabled={status!=='idle'} onClick={()=>setAmount(Math.max(.01,amount/2))} className="border-l border-white/5 px-3 text-xs font-bold text-slate-300 hover:bg-white/5">½</button><button disabled={status!=='idle'} onClick={()=>setAmount(Math.min(balance,amount*2))} className="border-l border-white/5 px-3 text-xs font-bold text-slate-300 hover:bg-white/5">2×</button></div>
  <div className="my-5 space-y-4">{children}</div>
  {potential && status==='active'?<div className="mb-3 flex justify-between rounded-lg bg-lime/10 px-3 py-2 text-xs"><span className="text-slate-400">Cashout value</span><b className="text-lime">${potential.toFixed(2)}</b></div>:null}
