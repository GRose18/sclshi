...
export default function Crash({onBack}:{onBack:()=>void}){
 ...
 const settleCashout=(at:number)=>{
  if(betCashedOut.current)return
 ...
 const start=(automaticTarget?:number)=>{
  ...
  const tick=(now:number)=>{const m=Math.exp((now-begun)/8500);liveMulti.current=m;draw(m);setMulti(m);if(m>=nextNoticeAt.current&&m<threshold.current)addBotCashout(m);if(automaticTarget&&automaticTarget<threshold.current&&m>=automaticTarget)settleCashout(automaticTarget);if(m>=threshold.current){liveMulti.current=threshold.current;setMulti(threshold.current);setCrashed(true);setStatus('idle');if(!betCashedOut.current)setRoundResult({profit:-amount});draw(threshold.current);return}raf.current=requestAnimationFrame(tick)}
  raf.current=requestAnimationFrame(tick)
 }
 const cashout=()=>{
  if(status!=='active'||betCashedOut.current)return
  settleCashout(liveMulti.current)
 }
 useEffect(()=>{draw(1);return()=>cancelAnimationFrame(raf.current)},[])
 const panel=<BetController amount={amount} setAmount={setAmount} status={status} onAction={status==='active'?cashout:()=>start()} onAutoAction={()=>start(autoCashout)} autoTarget={autoCashout} setAutoTarget={setAutoCashout} canCashOut={status==='active'} disabled={amount>balance} potential={amount*multi}><div className="rounded-lg bg-[#0c1623] p-3 text-xs text-slate-400">Cash out before the crash. Your cash-out settles immediately while the shared round keeps running.</div></BetController>
