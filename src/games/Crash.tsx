...
export default function Crash({onBack}:{onBack:()=>void}){
 const {debit,credit,balance}=useCasinoWallet(); const canvas=useRef<HTMLCanvasElement>(null);const raf=useRef(0);const [amount,setAmount]=useState(10);const [status,setStatus]=useState<'idle'|'active'>('idle');const [multi,setMulti]=useState(1);const [crashed,setCrashed]=useState(false);const threshold=useRef(2)
 ...
 const start=()=>{if(!debit(amount))return;threshold.current=crashPoint();setCrashed(false);setMulti(1);setStatus('active');const begun=performance.now();const tick=(now:number)=>{const m=Math.exp((now-begun)/8500);draw(m);setMulti(m);if(m>=threshold.current){setMulti(threshold.current);setCrashed(true);setStatus('idle');draw(threshold.current);return}raf.current=requestAnimationFrame(tick)};raf.current=requestAnimationFrame(tick)}
 const cashout=()=>{cancelAnimationFrame(raf.current);credit(amount*multi);setStatus('idle');setCrashed(false)}
